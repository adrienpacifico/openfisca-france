...
##### Dans l'ordre du revenu rev_sal à l'ir
 ...
 #          cho = simulation.calculate('cho', period) #TODO: gérer le chomage
            ...
            return period, salaire_imposable_mensuel*12 #+ cho
    ...
    class rng_mensuel_times_12(Reform.Variable):
        ...
        def function(self, simulation, period):
            ...
            csg_deduc = simulation.calculate_add('csg_deduc', period.this_year) #TODO : modify properly
            charges_deduc = simulation.calculate_add('charges_deduc', period.this_year) #TODO : modify properly
    ...
    class rfr_mensuel(Reform.Variable):
        reference = ir.rfr
        ...
        def function(self, simulation, period):
            ...
            f3va = self.sum_by_entity(f3va_holder)/12
            f3vi = self.sum_by_entity(f3vi_holder)/12
...
#######FIN
    ...
    class lambda_compensation(Reform.Variable):
        ...
        def function(self, simulation, period):
            ...
            impot_annuel = simulation.calculate_add("irpp_mensuel", period)
            impot_mensuel = simulation.calculate("irpp", period)
