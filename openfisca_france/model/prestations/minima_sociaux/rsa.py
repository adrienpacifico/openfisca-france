...
class rmi(DatedVariable):
    ...
    def function(self, simulation, period):
        ...
        activite = self.split_by_roles(activite_holder, roles = [CHEF, PART])
        condition_activite = (
            ((activite[CHEF] & activite[PART]) != 0)
            * ((activite[CHEF] & activite[PART]) != 2)
            * ((activite[CHEF] & activite[PART]) != 3)
        )
        ...
        return period,condition_activite * (
            max_(0, rsa_socle - rsa_forfait_logement - br_rmi))
        # TODO: Migré lors de la mensualisation, puis modifié pour que ça tourne. Probablement faux
...
class ppa_revenu_activite_individu(Variable):
    ...
    def function(self, simulation, period, reference_period):
        ...
        seuil_aah_activite = 29 * smic_horaire
...
class ppa_bonification(Variable):
    ...
    def function(self, simulation, period, reference_period):
        ...
        seuil_1 = 59 * smic_horaire
        seuil_2 = 95 * smic_horaire
...
class ppa(DatedVariable):
    ...
    @dated_function(start = date(2000, 1, 1))
    def function(self, simulation, period):
        period = period.this_month
        seuil_non_versement = 15
        ...
        ppa = simulation.calculate_add('ppa_fictive', period.this_month, extra_params = [period])
